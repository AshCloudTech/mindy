(function ($) {
  ...
  if ($filter.length > 0) {
    $filter.each(function () {
      var $this_li = $(this).find("li"),
        $that = $(this).next(".has-filtered");
...
const cards = Array.from(carousel.querySelectorAll(".boiler-card"));
const cardCount = cards.length;
let cardWidth = cards[0].offsetWidth + 25; // includes gap
...
// Manual scroll function (for buttons or auto-scroll)
function manualScroll(direction = 1) {
  ...
  setTimeout(() => {
    ...
    if (carousel.scrollLeft >= cardWidth * (cardCount * 2)) {
      carousel.scrollLeft -= cardWidth * cardCount;
...
// ===== AUTO SCROLL =====
const autoScrollInterval = 3000; // scroll every 3 seconds
